...
If your plugin registers a Nitro plugin, you can define your nitro plugin and then register it using your module.
```ts nitro.plugin.ts
// nitro.plugin.ts
...
export default defineNitroPlugin((nitroApp) => {
  ...
  // Do somthing with your Nitro plugin with
  nitroApp.hooks.hook("render:html", (html) => {
    console.log("Happy HTML Nitro Plugin")
  });
});

```

You can then register your Nitro server plugin from `nitro.plugin.ts`
```ts module.ts
...
export default defineNuxtModule({
  setup(options, nuxt) {
    ...
    addServerPlugin(resolve(runtimeDir, "nitro.plugin.ts"));
